...
The application of the lens equation in symmetrical Bragg geometry is discussed in \inred{appendix}~\ref{sec:BraggGeometry}. %with comparison to recent numerical simulations \cite{Honkanen2018}) which are based on a finite element method to take the propagation in the crystal into account.
...
\begin{figure}
\label{fig:vectors}
\caption{Schematic view of the relevant parameters in focusing by a bent crystal in Bragg geometry. \todo{$\varphi_0$ closer...}
}
...
and is proportional to the Bessel function \inred{$J_0(2\sqrt{\Omega s_0 s_h})=$} $J_0(Z\sqrt{a^2-\xi^2})$ in this interval \cite{kato1961}, with  $Z=k\sqrt{\gamma\chi_h\chi_{\bar h}}/\sin2\theta_B$. In the case $|Z a| \gg 1$ the asymptotic approximation
\begin{equation}
...
Equation~(\ref{eq:polychromaticfocusing}) is usually referred to as the "geometric focusing" condition for bent crystals. It is also applied in the case of flat crystals \cite{sanchezdelrio1994}. \inred{Like in equation~(\ref{eq:CLE}), the crysal thickness is not taken into account.}
...
The conditions (\ref{eq:CLE}) and (\ref{eq:polychromaticfocusing}) \inred{always} coincide, in both asymmetric Laue and Bragg cases, \inred{in} the Rowland condition $L_0=R_c\cos\varphi_0$, as they both give $L_h=R_c|\cos\varphi_h|$. 
...
\inred{
On synchrotron dispersive EXAFS beamlines the use of a Bragg symmetric reflection by a bent ``polychromator" at a large distance from the source garantees that a broad bandwidth (up to $~1$ keV) is focused onto a small spot \cite{Tolentino:ms0206} at a focal distance close to $L_h=R_c\cos\theta_B/2$. 
}

% \inblue{Synchrotron dispersive EXAFS beamlines, \inred{because} the objective is to reflect \inred{and focus} a large energy bandwidth (polychromator), \inred{work} therefore in off-Rowland condition with a large source distance. The use of symmetric Bragg reflection  guarantees that the polychromator focuses a broad-bandwidth (up to ~1 keV) beam onto a small spot \cite{Tolentino:ms0206} at a focal distance \inred{close to} $L_h=R_c\cos\theta_B/2$.}

Laue polychromators are also used in synchrotron beamlines.
As discussed \inred{above},
conditions (\ref{eq:CLE}) and (\ref{eq:polychromaticfocusing}) coincide only in Rowland configuration, \inred{meaning that it would be} impossible to work \inred{ in off-Rowland condition} to obtain a large energy bandwidth. 
...
\inred{which means} real focusing \inred{is obtained} at the distance $|L_h|=q_{dyn}/4$ with beam incidence in the \inted{crystal} convex side ($R_c<0$). 
...
\end{equation}
with solution $F_h(s'_0,s'_h) = i\frac{k}{2}\chi_h E \exp[-i A s'_0 s'_h]$. We obtain equation (\ref{eq:kummerapprox}) of section \ref{sec:dynamlicalLaue}.
}

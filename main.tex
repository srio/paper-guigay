...
in the case of symmetrical geometry. The theory was extended to the asymmetric case by \citeasnoun{Kohn2000}.
...
For symmetrical Laue geometry, the factor $\exp(i \chi_0 (s'_0+s'_h))$ in equation~(\ref{eq:kummer}) is constant\inred{, as well as $i(k/2)\chi_h$ and they will be omitted. The IF in equation~(\ref{eq:kummer}) is (see Appendix~\ref{appendix:TTEintegral})}
...
\end{equation}
Using the integration variable $\eta=\xi-\tau$, the wave amplitude along the \xi$-axis is
} 
...
$(F_0,F_h)=(E\delta(s'_h),0)$, we obtain the first-order term  $(F_0,F_h)=(E\delta(s'_h), i\frac{k}{2}\chi_{h}E)$. Successive terms are obtained by iteration. \inref{$F_0$ and $F_h$} are made up by terms of  even and odd order, respectively. Alternatively, we can combine equations (\ref{eq:TTEappendixIntegral}) into a single integral equation \inred{for $F_h$}
\begin{equation}

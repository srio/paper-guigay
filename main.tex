...
\title{\inred{\sout{Monochromatic}} X-ray focusing by bent crystals: focal positions as predicted by the crystal lens equation and the dynamical diffraction theory}
...
% The spatial position $\vec x$ are expressed in oblique coordinates $(s_0,s_h)$ along the directions of the $\vec k_0$ and $\vec k_h=\vec k_0 + \vec h$ vectors.
...
\inblue{The exponential term $\exp(i \vec k . \vec h(s_0,\sigma_h))$
 in equation~(\ref{eq:kummer}) can be interpreted as the phase shift acquired at the first scattering even which, indeed, takes place at the point of coordinates $(s_0,\sigma_h)$. It therefore represents the kinematical (single-scattering) approximation of the dynamical theory}
...
\begin{figure}
\label{fig:flatLaue}
\caption{Numerical evaluation of on-axis intensity for a  \SI{250}{\micro\meter} thick flat Si111 crystal ($R=\infty$) with source at the crystal entrance ($p=0$) calculated using equation~(\ref{eq:Fresnel}) \todo{refer to a more explicit equation!}.
...
The equation~\inred{(10???)} (\ref{eq:kummer}) can be written as
\begin{equation}
...
% 
% 8.3 keV
% Maxumum found at x=651.652, y=0.00129274
% Maxumum found at x=1330.33, y=0.000104183

% q1, q2:  652.9715164464284 1351.8163380748058

% 17 keV
% Maxumum found at x=624.625, y=0.00673069
% Maxumum found at x=1372.37, y=0.00485101
% q1, q2:  612.1938422465382 1393.7950258283208
%
\todo{For the case in Fig.~\ref{fig:8keV}, at 8.3 keV we obtained numerically $q_1$=~\SI{652}{\milli\meter},
$q_2$=~\SI{1330}{\milli\meter} and using equations (\ref{eq:q1andq2}) we get
$q_1$=~\SI{653}{\milli\meter},
$q_2$=~\SI{1351}{\milli\meter}. And for 17 keV 
we had from Fig.~\ref{fig:8keV}
$q_1$=~\SI{625}{\milli\meter},
$q_2$=~\SI{1372}{\milli\meter} and using equations (\ref{eq:q1andq2}) we get
$q_1$=~\SI{612}{\milli\meter},
$q_2$=~\SI{1394}{\milli\meter}.
}
...
\begin{figure}
\label{fig:8keV}
\caption{Numerical evaluation of diffracted intensity by a \SI{250}{\micro\meter} thick Si 111 symmetric Laue crystal calculated using equation~(\ref{eq:Dhpropagated}) for a bent (R~= \SI{1}{\meter}) crystal and $p$~= \SI{30}{\meter}\todo{CALCULATED FOR 50!!}. 
...
b) on-axis intensity for a at photon energy of 17 keV.
Inset: transverse profile at the focal distances (maximum values):
...
Consequently, equations (\ref{eq:TTEappendix}) are rewritten as
\begin{subequations}
...
\end{align}
\end{subequations}
where $\Omega=k^2\chi_h\chi_{\bar h}/4$. The refracted \inred{reflected} amplitude is $F_{ref}=E\delta(s'_h)$. Equations (\ref{eq:TTEappendix2}) can be put in the form of integral equations:
\begin{subequations}
...
$(F_0,F_h)=(E\delta(s'_h),0)$, we obtain the first-order term, or kinematical expression, $(F_0,F_h)=(E\delta(s'_h), E)$. Succesive terms are obtained by iteration. The transmitted and reflected amplitudes are made up by terms of  even and odd order, respectively. Alternatively, we can combine equations \inred{()} and \inred{(  )} into a single integral equation for the reflected amplitude
\begin{equation}
...
The corresponding expression for $F_0(s'_0,s'_h)$ is then easily obtained by using formula \inred{(  )}.
...
\begin{align}
    ...
    &\cos\alpha \sin2\alphs-\sin\alpha=\sin\alpha(2\cos^2\alpha-1)=\sin\alpha\cos\2\alpha. \nonumber
\end{align}
we obtain
\begin{align}

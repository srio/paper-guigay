...
The incident wave-amplitude is expressed in a plane perpendicular to the incidence direction at a negligible distance from the entrance surface (axis $\tau$ in Fig.~\ref{fig:vectors}).
Similarly, we will express the Bragg-diffracted amplitude $D_h$ in a plane perpendicular to the direction of the Bragg diffraction at a negligible distance from the exit surface (axis $\xi$ in Fig.~\ref{fig:vectors}).
...
%It will be extended in the next section to the bent crystal with a point source upstream from the crystal.
...
The reflected amplitude at any distance $q$ from the crystal can be calculated numerically, without the approximations used above, by propagating the wavefield at the crystal exit $D_h(\xi,0)=J_0(Z\sqrt{a^2-\xi^2})$ via convolution with the Fresnel propagator:
\begin{equation}
\label{eq:Fresnel}
...
For the case of symmetrical diffraction, we shall omit  the factor $\exp(i k \hic_0(s'_0+s'_h))$ which is then constant, and $A=0$, and the functions $\varphi_1(s_0)$, $\varphi_2(s_h)$, in equation (\ref{eq:cylinder}) are the same function, \inred{written now without subindices}, $\varphi(s_0)$, $\varphi(s_h)$}. The equation~(\ref{eq:preKummer}) can be written as
\begin{equation}
...
\todo{change abscissa label to $(\xi-\xi_c$ in b) and d). Bigger labels, Inset?}
}
\includegraphics[width=0.6\textwidth]{bent1m8keV.eps}
\includegraphics[width=0.39\textwidth]{bent1m8keV_profile.eps}
\includegraphics[width=0.6\textwidth]{bent1m17keV.eps}
\includegraphics[width=0.39\textwidth]{bent1m17keV_profile.eps}
\end{figure}

...
%In discussing the case of symmetrical geometry, we show the important role played by the ``length of dynamical focusing",  which depends on the parameters of the unbent crystal. The elements of the dynamical theory used in the present paper can be found in the textbook of \cite{authierbook}.
...
%is expressed as a function of a lateral spatial variable $\xi$, perpendicular to the direction of $\vec k_h$ and at negligible distance from the \inred{back side} of the crystal of thickness $t$ (see Fig.~\ref{fig:laue}). \inred{$D_h(\xi)$ is chosen to be the electric perturbance in the plane perpendicular to the propagation direction, to facilitate further propagation needed to find the focal position.}
...
\begin{tabular}{lccccccc}      % Alignment for each cell: l=left, c=center, r=right
...
% Example:  consider symmetrical Laue Si 111 reflection at photon energy 17 keV. A crystal with thickness $t$~= \SI{300}{\micro\meter}, $q_{dyn}$~= \SI{4480}{\mili\meter},  $q'_{dyn}$~= \SI{3350}{\mili\meter}.
...
% \todo{ REALLY REMOVE THIS INTRO? Let us first consider a point-source on the crystal surface ($p=0$). Since $A=0$ \todo{refer to definition of $A$} in Laue symmetrical case, the influence function is the unbent crystal influence function multiplied by the phase term $\exp(-i \varphi_1(s_0))=\exp(-i k u (u+a)/(2R'))$ \todo{use $\xi$ instead?  Why prime in $R$?}. It results that
% \begin{equation}
%     \frac{1}{q_{1,2}}=\frac{1}{R \cos\theta_B}\pm \frac{1}{q_{dyn}}
% \end{equation}
% }

%The IF for the bent crystal is the one for the unbent crystal multiplied by the phase term $\exp(-i \varphi_1(s_0))=\exp(-i k u (u+a)/(2R'))$. In the case of a source distant $p$ from the crystal, the focal positions verify the equation \cite{GuigayFerrero2013}
...
% It can be used with $q_{dyn}$ replaced by the numerically obtained effective $q'_{dyn}$ which is independent of the crystal curvature.
...
% The usual configuration of a EXAFS beam line is to obtain a real focus ()  from a very distant real sourcewe ().  and  are decreasing functions of  p. Suppose . Note that, as p increases from 0 to infinity,  decreases from  to . Simultaneously, decreases first from  to . 
% For very large p-values, we have the simple relation .
...
%Similar calculations are shown in Fig.~\ref{fig:17keV} for a \SI{250}{\micro\meter} thick crystal, \todo{check parameters 111 reflection now!} with the same curvature radius and source distance, but at an x-ray photon energy of 17 keV. 
...
% The highest peak is found for $q_1$ ($q_1<q_2$) at x-ray energy of 17 keV. At 8 keV, the  $q_1$ peak is present, and the $q_2$ peak is damped out because the anomalous absorption effect.
%$\operatorname{Im}(Z)$ is negative in these cases. 
%This means that the focal position with low absorption (therefore with large intensity) is $q_1$. This is in agreement with the numerical plots. 
...
% \begin{figure}
% \label{fig:17keV}
% \caption{Numerical evaluation of diffracted intensity by a \SI{250}{\micro\meter} thick Si 111 symmetric Laue crystal calculated using equation~(\ref{eq:Dh}) at photon energy of 17 keV ($a$~= \SI{48}{\micro\meter}).
%  a) on-axis intensity for a flat crystal ($R=\infty$) with source at the crystal entrance ($p=0$). The maximum is at \SI{2240}{\mili\meter} instead of $q_{dyn}$~=\SI{2877}{\mili\meter} predicted by equation~\ref{eq:qdyn}. b) on-axis intensity for a bent (R~= \SI{1}{\meter}) crystal and $p$~= \SI{30}{\meter}. c) transverse profile at the focus (maximum value in plot b) at \inred{$q_1$~= \SI{567.567}{\mili\meter}}

% a) on-axis intensity for a bent (R~= \SI{1}{\meter}) crystal.
% b) transverse profile at the two foci (maximum value found in a) at $q_1$~= \SI{626}{\mili\meter} (blue), and $q_2$~= \SI{1372.4}{\mili\meter} (green) }
% \includegraphics[width=0.95\textwidth]{bent1m17keV.eps}
% \includegraphics[width=0.95\textwidth]{bent1m17keV_profile.eps}
% \end{figure}

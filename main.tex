...
\begin{abstract}
The location of the beam focus produced when monochromatic x-ray radiation is diffracted by a bent crystal is predicted by \inred{a} crystal lens equation. We derive this equation on the basis of geometrical concepts. This equation has little utility for diffraction in Laue geometry. The \inred{focusing effect in} the Laue symmetrical case is discussed using concepts of dynamical theory and an extension of the lens equation is proposed. The existence of an additional polychromatic focus is analyzed and the feasibility of matching \inred{the } polychromatic and monochromatic \inred{focal positions} is discussed.   \\
...
% first  predicted in the case of flat crystals by \cite{AfanasevKohn1977} and then confirmed experimentally by \cite{Aristov1978, Aristov1980}. 
The \inred{dynamical} focusing of x-ray beams by Laue crystals
% as predicted by dynamical diffraction 
...
We analyze here the propagation of x-rays \inred{in} Laue crystals in the framework of the \inref{Takagi-Taupin equation, hereafter TTE} \inred{\cite{Takagi1962, Takagi, Taupin, Taupin1967}}. 
...
Dynamical focusing \inred{by} flat Laue crystals (without bending) \inred{was predicted by} \citeasnoun{AfanasevKohn1977} and verified experimentally by \citeasnoun{}{Aristov1978, Aristov1980}
in the case of symmetrical geometry. The theory was extended to the asymmetric case by \citeasnoun{Kohn2000}. \inred{The applications of dynamical focusing to high-resolution spectrometry was proposed by \citeasnoun{KohnGorobtsov2013}.}
...
\inred{The most basic case of dynamical focusing is that of a point-source in $O$ ($\sigma_0=\sigma_h=0$)} on the crystal entrance surface \inred{of the crystal of thickness $t$}.
...
with $a=t \sin2\theta_B/(2\cos\theta_1)$ and $\gamma=\cos\theta_1/\cos\theta_2$ (asymmetry ratio). 



The amplitude $D_h(\xi)$ is zero outside the interval $-a<\xi<a$,
% , with $a=t \sin2\theta_B/(2 \cos\theta_1)$, 
and is proportional to the Bessel function  $J_0(Z\sqrt{a^2-\xi^2})$ in this interval \cite{kato1961}, with  $Z=k\sqrt{\gamma\chi_h\chi_{\bar h}}/\sin2\theta_B$. In the case $|Z a| \gg 1$ the asymptotic approximation
\begin{equation}
...
\inred{Moreover, the moduli of the two terms in equation~(\ref{eq:approximatedDiffractedField}) are proportional to $\exp(\pm a \text{Im}~Z)$. This is the expression of anomalous absorption (Borrmann effect). Two focal positions will be observed for small absorption, but only one for strong absorption, as shown in Fig.~\ref{fig:flatLaue}
}
...
\inred{where
\begin{equation}
\label{eq:PostFresnel}
    D_h(\xi',0) = i \dfrac{k}{2} \chi_h 
    J_0(Z\sqrt{a^2-\xi'^2}).
\end{equation}
}
...
\begin{equation}
    p + q = q_{dyn}.
\end{equation}
On the contrary, propagation through a bent crystal is not space-invariant, for the following reason: the IF in a bent crystal case, the IF is not only dependent on the variables $(s'_0, s'_h)$, but also on $(\sigma_0, \sigma_h)$ because of the factor  
...
\inred{In} symmetrical Laue geometry, the factor $\exp(i \chi_0 (s'_0+s'_h))$ in equation~(\ref{eq:kummer}) is constant\inred{ and will be omitted. Equation~(\ref{eq:kummer}) is (see Appendix~\ref{appendix:TTEintegral})}
...
\end{equation}
where $\vec h . \vec u(s_0,\sigma_h)$ is given in Appendix~\ref{appendix:TTEintegral}.
% with 
% \begin{equation}
...
% \end{equation}
Let us consider the incident wave amplitude $D_{inc}(\tau)=\exp(i k \tau^2/(2p))$, where $\tau$ is a coordinate along the axis $O\tau$ normal to $\vec k_0$ (see Fig.~\ref{fig:laue}). \inred{On the exit surface, using $s_0=(\xi+a)/\sin2\theta$ and $\sigma_h=-\tau/\sin2\theta_B$, and the notation $R'=R\cos\theta_B$ we obtain

\begin{equation}
    \vec h . \vec u(s_0,\sigma_h) = k \frac{\tau(\tau+a)-\xi(\xi+a)}{ 2R'}.
\end{equation}
Using the integration variable $\eta=\xi-\tau$, the wave amplitude along the $\xi$-axis is, omitting $i(k/2)\chi_h$
}
...
\end{equation}
which is demonstrated in Appendix~\ref{appendix:Deformation}, the focusing condition is 
\begin{equation}
...
The equation $\varphi_0+\varphi_h=2\alpha$ in Laue case\inred{, or $\varphi_0+\varphi_h=2\alpha+\pi$ in Bragg case,} implies $\Delta\varphi_0+\Delta\varphi_h=0$. Using equations~(\ref{eq:angles}) and  (\ref{eq:angles2}) we obtain the polychromatic crystal lens equation
\begin{equation}
...
Equation~(\ref{eq:polychromaticfocusing}) is usually referred to as the "geometric focusing" condition for bent crystals, also applied in the case of flat crystals \cite{sanchezdelrio1994}. \inred{Just as for the monochromatic condition (\ref{eq:CLE}), the crysal thickness is not taken into account in equation (\ref{eq:polychromaticfocusing}).}


% . This polychromatic focusing effect was also discussed for flat Laue crystals \cite{sanchezdelrio1994}. Equation~(\ref{eq:polychromaticfocusing}) is applied for focusing a polychromatic beam by a thin crystal instead of the CLE. 
...
It is easy to see that \inred{conditions} (\ref{eq:CLE}) and (\ref{eq:polychromaticfocusing}) coincide in the symmetric Bragg case for which $\cos\varphi_o=-\cos\varphi_h=\sin\theta_B$. 
In the symmetric Laue case, for which $\cos\varphi_o=\cos\varphi_h=\cos\theta_B$, the  condition (\ref{eq:CLE}) is reduced to $L_0=L_h$ and equation~(\ref{eq:polychromaticfocusing}) is then $L_0=L_h=R_c\cos\theta_B$, which corresponds to Rowland geometry. Therefore \inred{conditions} (\ref{eq:CLE}) and (\ref{eq:polychromaticfocusing}) coincide \inred{in} symmetric Laue case only if the Rowland condition is satisfied.\footnote{\cite{CK} stated that they coincide if a symmetrical reflection is used {\it or} the Rowland condition is fulfilled, which is false in the symmetric Laue off-Rowland}

The conditions (\ref{eq:CLE}) and (\ref{eq:polychromaticfocusing}) do coincide, in both asymmetric Laue and Bragg cases, \inred{in} the Rowland condition $L_0=R_c\cos\varphi_0$, as they both give $L_h=R_c|\cos\varphi_h|$. 
A narrow energy band \inred{is reflected}, because the angle of incidence on the local reflecting planes does not change along the bent crystal surface.
Synchrotron dispersive EXAFS beamlines, \inred{for which }the objective is to reflect \inred{and focus} a large energy bandwidth (polychromator), \inred{work} therefore in off-Rowland condition with a large source distance. The use of symmetric Bragg reflection  guarantees that the polychromator focuses a broad-bandwidth (up to ~1 keV) beam onto a small spot \cite{Tolentino:ms0206} at a focal distance \inred{close to} $L_h=R_c\cos\theta_B/2$.

Laue polychromators are also used in synchrotron beamlines.
As discussed \inred{above},
conditions (\ref{eq:polychromaticfocusing}) and (\ref{eq:CLE}) coincide only in Rowland configuration, \inred{meaning that it would be} impossible to work \inred{ in off-Rowland condition} to obtain a large energy bandwidth. 
However, \inred{in symmetric Laue geometry condition (\ref{eq:CLE}) should be replaced by equation~(\ref{eq:newCLE}), which means $L_h \approx R_c \cos\theta_B + (R_c \cos\theta_B) ^2 / q_{dyn}$ if the source-to-crystal distance is very large.} 
The coincidence \inred{with (\ref{eq:polychromaticfocusing}) } is then realised if
% $R_c \cos\theta_B /2 = R_c \cos\theta_B+ (R_c \cos\theta_B) ^2 / q_{dyn}$ which implies 
$R_c=-q_{dyn}/(2\cos\theta_B)$.
\inred{Under this condition,} real focusing \inred{is obtained} at the distance $|L_h|=q_{dyn}/4$ with beam incidence in the convex side ($R_c<0$). 
\inred{If the distance $|L_h|$ is fixed, the required conditions are $|R_c| = 2 k_h / \cos\theta_B$ and $q_{dyn}=4|L_h|$. The last condition should be fulfilled by the adequate choice of} the crystal thickness, as in  \cite{Mocella2004,Mocella2008}.

% The Laue crystal can then be optimized to verify this condition by choosing the right $q_{dyn}$. This is done by selecting an adequate crystal thickness to obtain the required $q_{dyn}$ value \cite{Mocella2004} \cite{Mocella2008}. 

% 
% FOR LATER!!!!!!!!!!!!!!!!!!!!!
\todo{ TO BE EXTENDED... Another equation has been introduced in order to deal with the effects of a finite crystal thickness \cite{PengQi, PengQi2021} found another polychromatic condition in Laue geometry. }
...
\unred{Under} a deformation field $\vec u(\vec r)$, the crystal polarizability \inred{is} taken as $\chi(\vec r-\vec u(\vec r))$, where $\chi(\vec r)$ is the polarizability of the non-deformed crystal. The Fourier components of the electric susceptibility $\chi_{h,\bar h}$ are multiplied by the phase factors $\exp(-  \vec h . \vec u (\vec r))$ and $\exp(\mp i \vec h . \vec u (\vec r))$, respectively, in the TTE.
In the case of a very thin crystal, the incident ray at the \inred{position} $x$ on the \inred{bent} crystal surface is simply affected by the phase factor 
\begin{equation}
...
\inred{For a point source in position $(\sigma_0,\sigma_h)$, the incident wave has the form $\exp(i\vec k_0 . \vec x) \delta(s_h-\sigma_h)$.}
...
$(F_0,F_h)=(E\delta(s'_h),0)$, we obtain the first-order term  $(F_0,F_h)=(E\delta(s'_h), i\frac{k}{2}\chi_{h}E)$. Successive terms are obtained by iteration. \inref{$F_0$ and $F_h$} are made up by terms of  even and odd order, respectively. Alternatively, we can combine equations (\ref{eq:TTEappendixIntegral}) into a single integral equation \inred{for $F_h$ only}
\begin{equation}
...
\begin{equation}
    D_h = i \frac{k}{2} \chi_h \exp\left[  i \frac{k}{2} \chi_0 (s'_0+s'_h) - i \vec k . \vec u(s_0,s_h) \right] J_0(2\sqrt{\Omega s'_0 s'_h}).
\end{equation}


In the opposite case when $A >> \Omega$, an approximated solution could be obtained by considering the integral equation
\begin{equation}
        F_h(s'_0,s'_h) = i\frac{k}{2}\chi_h E - i A s'_0 \int_0^{s'_h} d\xi_h F_h(s'_0,\xi_h),
\end{equation}
with solutions $F_h(s'_0,s'_h) = i\frac{k}{2}\chi_h E \exp[-i A s'_0 s'_h]$. We obtain equation (\ref{eq:kummerapprox}) of section \ref{sec:dynamlicalLaue}.
}
...
\section{General expression of the phase factor and derivation of equation~(\ref{eq:appendixIdentity})}
...
\end{equation}
by some cumbersome algebraic manipulations the numerator of this expression is reduced to $\sin\alpha \sin^22\theta_B$ from where is easy to get equation (\ref{eq:appendixIdentity}).
}
